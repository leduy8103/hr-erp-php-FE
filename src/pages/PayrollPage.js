...
import { getAllPayrolls, deletePayroll } from '../services/payrollService';
  ...
  const fetchPayrolls = async () => {
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        const formattedPayrolls = response.data.map(payroll => {
          // Extract employee information from the response
          const employeeName = payroll.employee_name || payroll.full_name || `ID: ${payroll.employee_id}`;
          const employeeEmail = payroll.employee_email || 'N/A';
          ...
          return {
            ...
            employee: {
              ...
              email: employeeEmail, // Use the email from joined users table
              avatarURL: payroll.avatar_url || null,
        ...
        console.log("Formatted Payrolls:", formattedPayrolls); // Debug log
        setPayrolls(formattedPayrolls);
        setError("");
      } else {
        console.error("Invalid payroll data:", response);
        setPayrolls([]);
  ...
  const handlePayrollUpdated = (updatedPayroll) => {
    setPayrolls(prevPayrolls => 
        prevPayrolls.map(payroll => 
            payroll.id === updatedPayroll.id ? {
                ...updatedPayroll,
                employee: {
                    ...payroll.employee,
                    id: updatedPayroll.employee_id,
                    full_name: updatedPayroll.employee_name || payroll.employee.full_name,
                    email: updatedPayroll.employee_email || payroll.employee.email
                }
            } : payroll
        )
    );
